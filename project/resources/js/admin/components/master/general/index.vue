  ...
  export default {
    ...
    data() {
      return {
        ...
        message: "",
        emptyItem: null,
        dlMasterUrl: '/general/export_excel',
      }
    },
    computed: {
      sortOptions() {
    ...
    mounted() {
       this.isBusy = true;
       axios.post("/general/get_conds", {isInit: this.isInit})
        .then(response => {
    ...
    methods: {
      ...
      delete(item) {
        axios.delete("/general/" + item.id).then(response => {
          this.getItem();
          this.message = "";
        })
        .catch(error => {
