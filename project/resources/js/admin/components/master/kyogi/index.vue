  ...
  <b-container fluid class="py-4">
    ...
    <b-card
      ...
      <b-row>
        ...
        <b-col lg="6" class="my-1">
          <b-form-group label="会場名" label-for="kaizyo_nm">
            <b-input-group>
              <b-form-input v-model="conds.kaizyo_nm" type="search" id="kaizyo_nm"></b-form-input>
            </b-input-group>
          </b-form-group>
        </b-col>
      </b-row>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        items: [],
        conds: {kyogi_nm: null, kaizyo_nm: null},
        fields: [
          { key: 'kyogi_nm', label: '競技名', sortable: true, sortDirection: 'desc' },
          { key: 'address', label: '住所', sortable: true, class: 'text-left' },
        ...
        message: "",
        emptyItem: null,
        dlMasterUrl: '/kyogi/export_excel',
      }
    },
    computed: {
      sortOptions() {
    ...
    mounted() {
       this.isBusy = true;
       axios.post("/kyogi/get_conds", {isInit: this.isInit})
        .then(response => {
          if(!this.isEmptyObject(response.data)){
            this.conds.kyogi_nm = response.data.kyogi_nm;
            this.conds.kaizyo_nm = response.data.kaizyo_nm;
          }
          this.getItem();
        })
       this.getEmptyData();
    ...
    methods: {
      getItem() {
        // 条件をセッションに保存
        axios.post("/kyogi/set_conds", {
          kyogi_nm: this.conds.kyogi_nm,
          kaizyo_nm: this.conds.kaizyo_nm,
          isInit: this.isInit
        }).catch();

        // 一覧読込
        axios.post("/kyogi/index", {
          kyogi_nm: this.conds.kyogi_nm,
          kaizyo_nm: this.conds.kaizyo_nm,
          isInit: this.isInit
        })
      ...
      getEmptyData() {
        axios.post("/utility/empty_table_columns", {table_nm: 'kyogis', num: 1}).then(response => {
          this.emptyItem = response.data;
        })
      },
      edit(item, index, button) {
        this.$router.push({ name: "kyogi_edit" , params: {item : item, initialErrors : this.emptyItem}}).catch(() => {});
      },
      create() {
        let errors = $.extend(true, {}, this.emptyItem);
        this.$router.push({ name: "kyogi_create" , params: {item : this.emptyItem, initialErrors : errors}}).catch(() => {});
      },
      delete(item) {
        axios.delete(format.sprintf('/kyogi/%1$s', item.id)).then(response => {
          if(response.data.success){
            this.getItem();
            this.message = "";
          }else{
            this.message = "別のデータで使用されています。";
          }
        })
        .catch(error => {
          this.message = error;
      ...
      showDelConirm(item) {
        this.$bvModal.msgBoxConfirm(format.sprintf('%1$s を削除します。よろしいですか?', item.kyogi_nm))
          .then(result => {
